...
These are now **pinned in `server/classifier/requirements.txt`** — see below.

### 2a. Runtime dependency versions

Three distinct things must not be conflated:

#### Original training environment — mostly UNKNOWN

| Package | Training version | Evidence |
|---|---|---|
| **scikit-learn** | **1.7.0** — ✅ *established from the artifacts* | Loading `scaler.pkl` and the label encoders under a different version raises `InconsistentVersionWarning: Trying to unpickle estimator StandardScaler from version 1.7.0` |
| xgboost | **UNKNOWN**, older than 3.4.1 | Under 3.4.1 XGBoost warns the model was "generated by an older version". No exact version is recorded |
| lightgbm | **UNKNOWN**, 4.x | Booster string reports `version=v4`; the minor version is not recorded |
| pandas / numpy / joblib | **UNKNOWN** | Nothing in the repository records them |
| Python | **UNKNOWN** | `__pycache__/*.cpython-313.pyc` shows 3.13 ran *something* here; it is not evidence of the training interpreter |

> Only the scikit-learn version is provable from repository artifacts. Everything else is **UNKNOWN**
> and is documented as such rather than guessed.

#### Currently verified compatible runtime — what is pinned

Verified experimentally in a clean virtual environment on **Python 3.12.13**:

```
Flask==3.1.3   pandas==2.3.3   numpy==2.3.4   joblib==1.5.3
scikit-learn==1.7.0   xgboost==3.0.5   lightgbm==4.7.0   gunicorn==23.0.0
```
...
| Check | Result |
|---|---|
| `lightgbm_model.pkl`, `xgboost_model.pkl`, `scaler.pkl`, `feature_cols.pkl` load | ✅ all four, **zero warnings** |
| Representative prediction (15 d, 3269 ppm, 6 R⊕, 1.05 R☉) | ✅ `Candidate` — FP 9.41 % / Cand 88.2 % / Conf 2.39 % |
| Agreement with the developer environment | ✅ max abs probability difference **3.75e-11** |

`scikit-learn==1.7.0` is pinned because it *matches the version recorded inside the artifacts*, which
removes `InconsistentVersionWarning` entirely. `xgboost==3.0.5` and `lightgbm==4.7.0` are **not**
claimed to be the training versions — they are simply the versions verified here to load the
artifacts cleanly and reproduce the reference prediction.

#### Also known to work — the developer environment

`server/venv` (Python 3.14.2, scikit-learn 1.9.0, xgboost 3.4.1, lightgbm 4.7.0, pandas 3.0.5,
numpy 2.5.2) loads all four artifacts and produces the *same* prediction, but emits
`InconsistentVersionWarning` and an XGBoost old-format warning. Usable for development; the pinned
set is preferred for deployment.

**No model artifact was modified, retrained, or re-serialized.**
